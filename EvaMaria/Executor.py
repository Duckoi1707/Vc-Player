...
async def eval_(bot, message):
    try:
        ...
        return await bot.send_message(message.chat.id, "`🙄Lệnh không tìm thấy.`", reply_to_message_id=message.message_id)
    msg = await bot.send_message(message.chat.id, "`😼Thực thi eval...`", reply_to_message_id=message.message_id)
